...
async def test_auth_login():
    async with AsyncClient(base_url="http://127.0.0.1:8000") as client:
        response = await client.post("/auth/login", json={"email": "user@example.com", "password": "root1"})
...
async def test_logout():
    ...
    async with AsyncClient(base_url="http://127.0.0.1:8000", cookies=cookies) as async_client:
...
async def test_upd_user_by_id_no_auth():
    ...
    user_id = 1
    async with AsyncClient(base_url="http://127.0.0.1:8000") as async_client:
        response = await async_client.put(f"/users/update_by_id/{user_id}", json=updated_user)
    assert response.status_code == 307
