...
async def test_upd_customer_by_id_no_auth():
    ...
    customer_id = 1
    async with AsyncClient(base_url="http://127.0.0.1:8000") as async_client:
        response = await async_client.put(f"/customers/update_by_id/{customer_id}", json=updated_customer)
    assert response.status_code == 307   
...
async def test_upd_customer_by_filter_no_auth():
    ...
    async with AsyncClient(base_url="http://127.0.0.1:8000") as async_client:   
        response = await async_client.put("/customers/update_by_filter/", json={**updated_customer, **filter_data})
    assert response.status_code == 307    
