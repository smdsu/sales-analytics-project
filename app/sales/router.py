...
from app.users.dependencies import is_current_user_admin, is_current_user_analyst, is_current_user_vendor   
...
async def get_all_sales(request_body: RBSale = Depends(), user_data: User = Depends(is_current_user_analyst)) -> list[SSale]:
...
async def get_sale_by_id(id: int, user_data: User = Depends(is_current_user_analyst)) -> SSale | None:
...
@router.post("/add/")
async def add_sale(sale: SSaleAdd, user_data: User = Depends(is_current_user_vendor)) -> dict:
...
async def update_sale_by_id(id: int, new_sale: SSaleUpd = Depends(), user_data: User = Depends(is_current_user_admin)) -> dict:
...
async def update_sale_by_filter(new_sale: SSaleUpd = Depends(), user_data: User = Depends(is_current_user_admin)) -> dict:
...
async def delete_sale_by_id(id: int, user_data: User = Depends(is_current_user_admin)) -> dict:
