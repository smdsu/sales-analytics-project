...
async def get_all_saledetails(request_body: RBSaleDetail = Depends(), user_data: User = Depends(is_current_user_analyst)) -> list[SSaleDetail]:
...
async def get_full_by_sale_id(sale_id: int, user_data: User = Depends(is_current_user_analyst)) -> list[SSaleDetail] | None:
...
@router.post("/add/")
async def add_saledetail(saledetail: SSaleDetailAdd, user_data: User = Depends(is_current_user_vendor)) -> dict:
...
@router.put("/update/")
async def update_saledetail(saledetail: SSaleDetailUpd, user_data: User = Depends(is_current_user_admin)) -> dict:
