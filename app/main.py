...
from app.exceptions import TokenExpiredException, TokenNotFoundException, NoUserIdException, NoUserException, NoJwtException
...
async def no_user_id_exception_handler(request: Request, exc: NoUserIdException):
    logger.error(exc)
    return RedirectResponse(url="/auth/")

@app.exception_handler(NoUserException)
async def no_user_exception_handler(request: Request, exc: NoUserException):
    logger.error(exc)
    return RedirectResponse(url="/auth/")

@app.exception_handler(NoJwtException)
async def no_jwt_exception_handler(request: Request, exc: NoJwtException):
    logger.error(exc)
    return RedirectResponse(url="/auth/")